...
<head>
  ...
  <title>Tulis Artikel Baru (Gutenberg Editor) - Both Corner</title>
...
<body class="bg-white text-slate-900 overflow-x-hidden antialiased flex flex-col h-screen">
  ...
  <form action="{{ route('developer.articles.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col h-full">
    @csrf
    ...
    <header class="h-16 border-b border-slate-200 px-6 flex items-center justify-between shrink-0 bg-white z-50">
      <div class="flex items-center gap-3">
        ...
        <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">Draft</span>
      </div>
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <div class="flex-1 overflow-y-auto px-10 py-12 flex justify-center bg-white">
        <div class="max-w-2xl w-full flex flex-col">
          ...
          <input type="text" name="title" id="post-title" required placeholder="Add title" oninput="updateSlugAndPreview()" class="text-4xl sm:text-5xl font-extrabold focus:outline-none placeholder-slate-300 bg-transparent w-full border-0 border-b border-transparent focus:border-transparent focus:ring-0 p-0 mb-6 tracking-tight text-slate-900">
          ...
          <textarea name="content" id="post-content" required placeholder="Type / to choose a block" class="flex-1 w-full border-0 focus:ring-0 outline-none p-0 resize-none text-slate-700 text-sm sm:text-base leading-relaxed focus:outline-none" style="min-height: 300px;"></textarea>

        </div>
      </div>

      <!-- Right Column: Sidebar Settings (Post Settings + Yoast/RankMath SEO) -->
      <aside class="w-80 border-l border-slate-200 overflow-y-auto bg-slate-50 flex flex-col shrink-0">
        ...
        <div id="sidebar-tab-post" class="p-6 space-y-6 block">
          ...
          <div class="flex flex-col gap-1.5 text-xs">
            ...
            <div class="border-2 border-dashed border-slate-200 rounded-xl p-4 bg-white flex flex-col items-center justify-center text-center cursor-pointer hover:border-indigo-500 transition-colors relative h-36" onclick="triggerFeaturedImageUpload()">
              ...
              <div id="featured-image-placeholder" class="space-y-2">
                <span class="text-2xl block">📁</span>
                <span class="block text-[10px] text-slate-500 font-bold">Pilih file gambar unggulan</span>
              </div>
              <img id="featured-image-preview" class="hidden max-h-full max-w-full rounded-lg object-contain">
            </div>
          ...
          <div class="flex flex-col gap-1.5 text-xs border-t border-slate-200 pt-4">
            ...
            <input type="text" name="slug" id="post-slug" required placeholder="url-slug-artikel" class="w-full px-3 py-2 rounded-xl border border-slate-200 bg-white outline-none focus:border-indigo-500 text-slate-700 font-mono transition-colors">
        ...
        <div id="sidebar-tab-seo" class="p-6 space-y-6 hidden">
          ...
          <div class="flex flex-col gap-1.5 text-xs">
            ...
            <input type="text" name="focus_keyword" id="seo-keyword" placeholder="Contoh: compatible device dslr" class="w-full px-3 py-2 rounded-xl border border-slate-200 bg-white outline-none focus:border-indigo-500 text-slate-700 transition-colors">
          ...
          <div class="flex flex-col gap-1.5 text-xs">
            ...
            <input type="text" name="seo_title" id="seo-title" placeholder="Google Search Title Preview" oninput="updateGooglePreview()" class="w-full px-3 py-2 rounded-xl border border-slate-200 bg-white outline-none focus:border-indigo-500 text-slate-700 transition-colors">
          </div>

          <!-- Meta Description -->
          <div class="flex flex-col gap-1.5 text-xs">
            <label class="font-bold text-slate-700">Meta Description</label>
            <textarea name="meta_description" id="seo-desc" rows="4" placeholder="Deskripsi meta..." oninput="updateGooglePreview()" class="w-full px-3 py-2 rounded-xl border border-slate-200 bg-white outline-none focus:border-indigo-500 text-slate-700 transition-colors leading-normal"></textarea>
          </div>
          ...
          <div class="border border-slate-200 rounded-xl bg-white p-4 shadow-sm text-xs">
            ...
            <div class="space-y-1">
              <span class="text-slate-400 text-[10px] block truncate">bothcorner.com > blog > <span id="preview-url-slug" class="font-mono">post-slug</span></span>
              <a href="#" class="text-blue-600 hover:underline text-sm font-semibold leading-tight block" id="preview-seo-title">Google Search Title Preview</a>
              <p class="text-slate-600 text-[11px] leading-relaxed line-clamp-2" id="preview-seo-desc">Deskripsi meta penelusuran yang disesuaikan untuk search engine optimization...</p>
            </div>
          </div>
  ...
  <script>
    ...
    async function uploadInlineImageFile(input) {
      ...
      // Reset input value so it can be triggered again
      input.value = '';
    }
