...
<head>
  ...
  <title>Edit Event: {{ $event->name }} - Both Corner Cloud</title>
...
<body class="bg-slate-100 text-slate-900 antialiased">
  @php
    ...
    $assignedTemplateIds = $event->eventTemplates->pluck('photobooth_template_id')->map(fn ($id) => (int)$id)->toArray();
    ...
    if (empty($enabledModes)) { $enabledModes = ['photo']; }
    $selectedTemplateIdsForForm = array_map('intval', old('templates', $assignedTemplateIds));
    $defaultTemplateIdForForm = old('default_template_id', $defaultTemplateId);
  @endphp
    ...
    <main class="flex-1 flex flex-col min-w-0">
      <header class="h-20 bg-white border-b border-slate-200/60 px-8 flex items-center justify-between shrink-0">
        <div class="min-w-0">
          ...
          <p class="text-xs text-slate-500 mt-1">Ubah event info, mode, layout, timing, dan sharing.</p>
        </div>
        <a href="{{ route('client.events.show', $event) }}" class="text-xs font-bold text-indigo-600 hover:underline">← Back to Event</a>
      </header>

      <div class="p-8 max-w-5xl">
        @if(session('success'))
          <div class="mb-6 p-4 rounded-2xl border border-emerald-200 bg-emerald-50 text-sm text-emerald-700 font-semibold">{{ session('success') }}</div>
        @endif
        @if($errors->any())
          <div class="mb-6 p-4 rounded-2xl border border-rose-200 bg-rose-50 text-sm text-rose-700 font-semibold">{{ $errors->first() }}</div>
        @endif

        @if($event->gallery_visibility === 'public')
          <div class="mb-6 p-4 rounded-2xl border border-indigo-200 bg-indigo-50 text-sm text-indigo-800">
            <span class="font-bold">Public URL:</span>
            <a href="{{ url('/e/' . $event->slug) }}" target="_blank" class="font-bold underline">{{ url('/e/' . $event->slug) }}</a>
          </div>
        @endif

        <form action="{{ route('client.events.update', $event) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200/70 shadow-sm overflow-hidden">
          @csrf
          @method('PUT')
          <div class="p-6 space-y-8">
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select id="status" name="status" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm bg-white outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100">
                      @foreach(['draft' => 'Draft', 'active' => 'Active', 'completed' => 'Completed'] as $value => $label)
            ...
            <section class="space-y-4 pt-4 border-t border-slate-100">
              ...
              @php($oldModes = old('capture_modes', $enabledModes))
            ...
            <section class="space-y-4 pt-4 border-t border-slate-100">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($templates as $template)
                  @php($isSelected = in_array((int)$template->id, $selectedTemplateIdsForForm, true))
                  <div class="p-4 border rounded-xl flex items-center justify-between gap-4 {{ $isSelected ? 'border-indigo-100 bg-indigo-50/30' : 'border-slate-200 bg-white' }}">
                    <label class="flex items-center gap-3 min-w-0 cursor-pointer">
                      <input type="checkbox" name="templates[]" value="{{ $template->id }}" {{ $isSelected ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600 border-slate-300">
                      <span class="min-w-0">
                    ...
                    <label class="flex items-center gap-1.5 shrink-0 cursor-pointer">
                      <input type="radio" name="default_template_id" value="{{ $template->id }}" {{ (string)$defaultTemplateIdForForm === (string)$template->id ? 'checked' : '' }} class="w-3.5 h-3.5 text-indigo-600 border-slate-300">
            ...
            <section class="space-y-4 pt-4 border-t border-slate-100">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-5 gap-4">
                ...
                <div>
                  ...
                  <input id="canvas_width" name="canvas_width" type="number" value="{{ old('canvas_width', optional($setting?->template)->canvas_width ?? 1200) }}" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm">
                </div>
                <div>
                  <label class="block text-xs font-bold text-slate-600 mb-2" for="canvas_height">Height</label>
                  <input id="canvas_height" name="canvas_height" type="number" value="{{ old('canvas_height', optional($setting?->template)->canvas_height ?? 1800) }}" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm">
                </div>
                <div>
                  <label class="block text-xs font-bold text-slate-600 mb-2" for="capture_count">Capture Count</label>
                  <input id="capture_count" name="capture_count" type="number" min="1" max="10" value="{{ old('capture_count', optional($setting)->capture_count ?? 3) }}" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm">
                </div>
              </div>
              <input type="hidden" name="layout_type" value="{{ old('layout_type', optional($setting)->layout_type ?? 'custom') }}">
              <div class="flex flex-wrap gap-6">
                <label class="flex items-center gap-3 cursor-pointer"><input type="checkbox" name="retake_enabled" value="1" @checked(old('retake_enabled', optional($setting)->retake_enabled ?? true)) class="w-4 h-4 rounded text-indigo-600 border-slate-300"><span class="text-xs font-bold text-slate-700">Retake</span></label>
                <label class="flex items-center gap-3 cursor-pointer"><input type="checkbox" name="print_enabled" value="1" @checked(old('print_enabled', optional($setting)->print_enabled ?? false)) class="w-4 h-4 rounded text-indigo-600 border-slate-300"><span class="text-xs font-bold text-slate-700">Print</span></label>
                <label class="flex items-center gap-3 cursor-pointer"><input type="checkbox" name="watermark_enabled" value="1" @checked(old('watermark_enabled', optional($setting)->watermark_enabled ?? false)) class="w-4 h-4 rounded text-indigo-600 border-slate-300"><span class="text-xs font-bold text-slate-700">Watermark</span></label>
              </div>
            </section>
          </div>

          <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3">
            <a href="{{ route('client.events.show', $event) }}" class="px-5 py-3 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-200/60">Cancel</a>
            <button type="submit" class="px-5 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-lg shadow-indigo-600/15">Save Changes</button>
          </div>
        </form>
      </div>
    </main>
